...
using RABFirstLesson.Common;

namespace RABFirstLesson
{
    [Transaction(TransactionMode.Manual)]

    public class DimensionMultipleVersionTwo : IExternalCommand
    {
        ...
        private XYZ GetOffsetByWallOrientation(XYZ point, XYZ orientation, int v)
        {
            XYZ newVector = orientation.Multiply(v);
        ...
        {
            Left = 0, CenterLR = 1, Right = 2, Front = 3, CenterFB = 4, Back = 5, Bottom = 6, CetnerElevation = 7, Top = 8
        }
                ...
                if (gInst != null)
                {
                    ...
                    if (gSymbol != null)
                    {
                        foreach (GeometryObject geomObj in gSymbol)
                        {
                            ...
                            else if (geomObj is Autodesk.Revit.DB.Point)
                            {
                                Autodesk.Revit.DB.Point point = geomObj as Autodesk.Revit.DB.Point;
                    ...
                    if (sampleStableRef != null)
                    {
                        ...
                        String customStableRef = refTokens[0] + ":"
                            + refTokens[1] + ":" + refTokens[2] + ":" + refTokens[3] + ":" + idx.ToString();
        ...
        private List<Solid> GetSolids(Element selectedElem)
        {
            ...
            GeometryElement geomElem = selectedElem.get_Geometry(option);
